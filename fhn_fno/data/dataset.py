...
class FHNOperatorDataset(Dataset):
    ...
    def __init__(self, data_file: str, mode: str = "single_step",
                 normalize: bool = True, train: bool = True,
                 train_split: float = 0.8, device: str = "cpu"):
        """mode: "single_step" gives (t -> t+1) pairs, "fixed_horizon" gives (t=0 -> t=T)."""
        self.mode = mode
        self.normalize = normalize
        self.device = device
        
        with h5py.File(data_file, 'r') as f:
            self.u_traj = np.array(f['u_traj'])  # (n_samples, n_times, *spatial)
            self.v_traj = np.array(f['v_traj'])
            self.params = np.array(f['params'])  # (n_samples, 5)
            self.I_ext = np.array(f['I_ext'])
        ...
        if normalize and train:
            ...
            # placeholder, should reuse the train-split stats here
            self.u_mean = 0.0
            self.u_std = 1.0
            self.v_mean = 0.0
            self.v_std = 1.0
    ...
    def denormalize(self, u: torch.Tensor, v: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.normalize:

...
def relative_l2_error(pred: torch.Tensor, target: torch.Tensor, 
...
def benchmark_inference(model: nn.Module, input_shape: Tuple[int, ...], 
                        device: str = 'cpu', n_runs: int = 100) -> dict:
    """Time forward passes, returns stats in ms."""
    model.eval()
...
def compute_pde_residual(u: torch.Tensor, v: torch.Tensor, 
                         params: dict, dx: float = 1.0) -> Tuple[torch.Tensor, torch.Tensor]:
    """FHN residuals via finite differences. Placeholder, spatial terms only."""
    if len(u.shape) == 2:  # 1D
...
def evaluate_model_performance():
    """Evaluate a checkpoint on test samples and benchmark inference."""
    # edit these directly, no CLI args
    DATA_FILE = "data/fhn_1d_tiny.h5"
    CHECKPOINT_FILE = "checkpoints/best_model.pt"
    DEVICE = "cpu"
    N_SAMPLES = 10
    INPUT_SHAPE = (4, 2, 256)  # (batch_size, channels, nx)
    N_BENCHMARK_RUNS = 50
